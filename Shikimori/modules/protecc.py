...
from sqlalchemy.sql.waifu_sql import is_harem, add_grp, rm_grp, get_all_grp
from sqlalchemy.sql.husb_sql import is_husb, add_hus_grp, rm_hus_grp, get_all_hus_grp
from . import *
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        OUTPUT_STR = """/protecc {prs_text}""".format(
            **locals())
        await rishu.edit(OUTPUT_STR, parse_mode="HTML", link_preview=False)
        ...
        if int(grps.chat_id) == event.chat_id:
            try:
                ...
                try:
                    ...
                    pass
                rishu = await event.client.send_message(event.chat_id, f"/protecc {text}")
                await sleep(2)
                await rishu.delete()
                os.remove(dl)
            except:
